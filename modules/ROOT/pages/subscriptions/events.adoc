...
In this occasion, a new event is triggered for each new node, containing the its properties.

This action is performed with the top-level subscription `[type]Created`, which contains the following fields:

* `event`: the event triggering this subscription (in this case, it is always `CREATE`).
...
If a same query triggers multiple events, it will come with the same timestamp.
...
A subscription to any `Movie` created should look like this:
...
Subscription to `UPDATE` events listen *only* to node properties changes, not updates to other fields.
...
* `event`: the event triggering this subscription (in this case, it is always `UPDATE`).
...
A subscription to any `Movie` updated should look like this:
