...
The following comparison operators are available for numeric types (`Int`, `Float`, xref::type-definitions/default.adoc[`BigInt`])
...
NOTE: Filtering on xref::type-definitions/temporal.adoc[Temporal Types] and xref::type-definitions/spatial.adoc[Spatial Types]: is not yet supported.
