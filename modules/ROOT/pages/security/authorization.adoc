...
[WARNING]
====
There is no `UPDATE_RELATIONSHIP` authorization operation, which means that `@authorization` *does not* protect mutations that update only relationship properties.

For example, with the following typeDefs:
...
extend type Movie @authorization(filter: [{ where: { node: { ownerId: { eq: "$jwt.sub" } } } }])
----

When executing the following query, an unauthorized user is able to update relationship properties:

[source, graphql, indent=0]
----
mutation {
  updateMovies(
    where: { title: { eq: "VictimMovie" } }
    update: {
      actors: [{
        update: { where: { node: { name: { eq: "Keanu" } } }, edge: { role: { set: "attacker-controlled-role" } } }
      }]
    }
  ) {
...
For this reason it's advised to avoid storing sensitive data in relationship
properties. You can also prevent updating relationship properties with the @settable directive:
...
type ActedIn @relationshipProperties {
    role: String @settable(onUpdate: false)
}
