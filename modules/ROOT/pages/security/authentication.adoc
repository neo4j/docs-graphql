...
* The `password` field is present in a GraphQL selection set.
...
You can perform additional checks against JWT claims together with authentication. 
For instance, if it is a requirement that only users with the `admin` role can delete users:
