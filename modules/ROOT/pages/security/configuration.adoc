...
To configure the library with a symmetric secret (e.g. "secret"), use the following instantiation:
...
To configure the library to verify tokens against a JSON Web Key Set (JWKS) endpoint, for example "https://www.example.com/.well-known/jwks.json", use the following instantiation:
...
For example, an HTTP request with the following `authorization` header should look like this:
