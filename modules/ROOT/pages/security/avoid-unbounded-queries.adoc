...
Using the xref::/directives/custom-logic.adoc#_limit[`@limit`] directive on a type definition allows to set a default value and a maximum for the limit on the number of nodes returned for that type from the server side.
...
* All queries which return arrays must include a `limit` argument.
...
This is an invalid query because the `limit` argument is not provided on the `movies` field:
...
This is an invalid query, because the `limit` argument is not provided on the `actors` field:
...
* If a field returns a list, its complexity is calculated by first adding the complexity values of its children and then multiplying it with the `limit` argument provided on that field.
* If no `limit` argument is provided, the default multiplier of `1` is used.

The following queries demonstrate the usage of the formula.

.Simple query example with `limit` arguments
====
[source, graphql, indent=0]
----
query {
...
.Query without `limit` arguments, using default multiplier of 1
====
...
.Deeply nested query example with `limit` arguments and fragments
====
...
This feature is intended to be used in conjunction with xref::neo4jgraphql-class.adoc#_limit_required_settings[`limitRequired`] to provide the most effective protection against unbounded queries. 
...
When `limitRequired` is not enabled, the complexity score is orders of magnitude lower since it uses the default multiplier of `1` for fields that return lists.
...
An alternative to computing the query complexity is to use a tool such as link:https://escape.tech/graphql-armor/docs/plugins/cost-limit/[GraphQL Armor].