...
`posts` and `users` are fields provided by the entity API, while `postsConnection` and `usersConnection` are provided by the connections API.
See xref:entity-and-connections-api.adoc[] for more information about the two.
