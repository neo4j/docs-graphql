...
These are the operators available for numeric (`Int`, `Float`, xref::type-definitions/default.adoc[`BigInt`]), xref::type-definitions/temporal.adoc[temporal] and xref::type-definitions/spatial.adoc[spatial] types:
...
See xref::/type-definitions/spatial.adoc#_filtering[Spatial types filtering] for more information.
