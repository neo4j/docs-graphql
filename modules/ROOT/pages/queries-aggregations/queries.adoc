...
All xref:type-definitions/relationships.adoc[relationships] are created with a direction from one node to another.
...
The xref:type-definitions/relationships.adoc#_querydirection[type definitions] for a relationship may define a different behavior, so the `directed` option may not be available in some cases.