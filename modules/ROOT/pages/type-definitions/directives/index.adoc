...
| Require authentication checks when accessing the type.

| xref::/authentication-and-authorization/authorization.adoc[`@authorization`]
| Specify authorization rules for queries and mutations on the type.
...
| `@relayId`
| Specify that the field should be used as the global node identifier for Relay.
...
| Specify authorization rules for subscriptions on the type.
