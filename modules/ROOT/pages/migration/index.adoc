...
|`nanoid`
|Changed to the `features` constructor object and part of `@callback`.
a|
.Before
[source, javascript, indent=0]
----
...
|`@plural`
|Removed from `@node` and rplaced by the `@plural` directive.
...
[discrete]
=== *`aggregate` value now defaults to false*

In version 4.0.0, the default value of the aggregate argument is *false*.
This means that aggregation operation fields are no longer generated by default when a relationship is defined using the `@relationship` directive.
...
These no longer generate `actedInAggregate` for the type `Actor`.
