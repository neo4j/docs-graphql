...
No change. See xref::/directives/cypher.adoc[`@cypher` directive] for more details on this directive in `@neo4j/graphql`.
...
There is an equivalent directive in the new library, but it does not work using database constraints as per the old library. See xref::/directives/autogeneration.adoc#type-definitions-autogeneration-id[`@id`].
...
Supported as you would expect, with additional xref::type-definitions/default.adoc[`BigInt`] support for 64 bit integers.
...
See xref::type-definitions/temporal.adoc[Temporal Types].

==== Spatial Types

The single type in `neo4j-graphql-js`, `Point`, has been split out into two types:

* xref::type-definitions/spatial.adoc#_point[`Point`]
* xref::type-definitions/spatial.adoc#_cartesianpoint[`CartesianPoint`]
...
Supported, queryable using inline fragments as per `neo4j-graphql-js`, but can also be created using Nested Mutations. See xref::type-definitions/interfaces.adoc[Interfaces].
