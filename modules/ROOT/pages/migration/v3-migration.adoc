...
please update your clients to use the new query names or use the `plural` option in the xref::/directives/database-mapping.adoc#type-definitions-node[@node directive]
...
sure to check the up-to-date documentation on xref::/directives/custom-directives.adoc[custom directives].
