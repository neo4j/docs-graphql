...
Example of using the nested operations can be found in xref::/mutations/create.adoc#_inserting_data[Relationships -> Inserting data].
...
This directive defines fields that can be used to group query results in a connection.
...
Adding a `@groupBy` directive to a field will enable `groupBy` operations inside connection queries. These will group the results by the provided fields:
...
Check the usage of `groupBy` xref::/queries-aggregations/group-by.adoc[here].
