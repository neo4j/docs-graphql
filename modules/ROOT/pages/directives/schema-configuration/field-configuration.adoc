...
Now the field `groupBy` exists in the `movieConnection` operation. This fields allow to query movies grouped by the `year` field.


The groupBy field, has one argument `fields`, that defines the field to group by. In this case `year` is the only option
...
The `groupBy` field will return an list of results, each result will contain a list of `edges`, with all the results grouped by the `year` field. For example:
...
Note that the groupedBy field `year` will be repeated on every node of the edges from each `groupBy` result.
...
In this case, all movies with the same `year` and `rating` will be grouped together in the same result.
