...
If both sides are non-list types the relationship is a xref:relationships/cardinality.adoc#_one_to_one_relationships[one-to-one] relationship and a xref:relationships/cardinality.adoc#_one_to_many_relationships[one-to-many] relationship otherwise.
...
Cosnider the following graph where a `Person` can direct multiple movies but a `Movie` only has one director. 
...
image::relationships/2-relationship-types.png[title="Model of a Movie node connected to a Person node through the `ACTED_IN` and `DIRECTED` relationships"]
...
.Get `Movie` nodes by title with related `Person` nodes via the `DIRECTED` relationship
====
[source, graphql, indent=0]
----
query {
...
.Get `Movie` nodes filtered by the name of the director via the `DIRECTED` relationship
====
[source, graphql, indent=0]
----
query {
...
.Get `Movie` nodes with a `DIRECTED` relationship to a `Person` node 
====
[source, graphql, indent=0]
----
query {
...
.Creating a `Movie` node and connecting it to a `Person` node relationship in a single operation
====
...
.Creating a `Movie` node and connecting it to an existing `Person` node
====
...
.Deleting `Movie` and `Person` nodes that are connected via the `DIRECTED` relationship
====
...
According to the xref:#_data_model[data model], it is impossible for a `Movie` to not have a director. 
Ideally this would be reflected by making the `director` field non-nullable.
...
image::relationships/data/2Person-Movie-single-relationship.png[title="Disconnected nodes and the same nodes with `DIRECTED` relationships"]

.Connecting two `Person` nodes to a `Movie` node via the `DIRECTED` relationship
====
[source, graphql, indent=0]
----
mutation {
...
Querying `Movies` and their `director` relationship fields returns only one object because of the type of the relationship in the GraphQL schema.
...
.Querying a `Movie` by title and the now ambiguous director
====
[source, graphql, indent=0]
----
query {
