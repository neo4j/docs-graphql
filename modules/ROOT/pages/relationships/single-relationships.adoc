...
However, many users of GraphQL wish to model relationships as either one-to-one or one-to-many in the GraphQL schema, therefore we have chosen to support this in the Neo4j GraphQL Library. 

Users of this feature should be aware there is no guarantee that only one relationship will be present in the database and this may result in non-deterministic behaviour. 
The only guarantee is that only one result will be returned when querying across a single relationship, but that result can be <<_at_most_one_relationship,any of the related nodes>>.
...
If the database contains multiple relationships of the same type, which is possible as a consequence of the Neo4j cardinality limitation, only one node is returned. See more details in <<_cardinality_limitations,Cardinality limitations>>.
...
Because of the current lack of constraints on relationship cardinality in the Neo4j database, it is not possible to enforce this cardinality of exactly one relationship and so all single relationship fields must be nullable.
...
In the context of multiple surfaces accessing the same Neo4j database through different APIs or direct database access, it is possible that what a GraphQL API models as a single relationship between two types ends up matching multiple relationships in the database, due to the current lack of constraints on relationship cardinality.
