...
[NOTE]
====
There is a caveat: the database only contains one _friends_ relationship, directed from Bob to Camilla. This means that the top level update operation has to be performed on Bob's node, as Camilla's node does not meet the filter criteria for the nested update operation.
