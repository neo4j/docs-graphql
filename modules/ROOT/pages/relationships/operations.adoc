...
mutation {
    createPeople(input: [{ 
        ...
        friends: { connect: { where: { node: { name: "Bob" } } } }, 
...
mutation {
    updatePeople(
        ...
        update: {
            ...
            friends: { 
                connect: { where: { node: { name: "Charlie" } } },
                create: { node: { name: "Camilla" } }
            },
            acquaintances: { connect: { where: { node: { name: "Dave" } } } }
        }
    ) {
        name
        friends {
...
Since their nodes are related through the _friends_ relationship, you can perform both changes in the same mutation starting with a top level update operation on either nodes and a nested update operation for the other one.
...
There has to be a relationship of type HAS_FRIEND between Bob and Alice in order for the node Alice to meet the filter criteria for the nested delete operation, and the same for Charlie and the nested disconnect operation.
...
Since the nodes are related through the _friends_ relationship, both deletions can be performed in the same mutation starting with a delete operation on either nodes and a nested delete operation for the other one.

.Top level delete with nested delete operation
====
[source, graphql, indent=0]
----
query {
    deletePeople(
        ...
        delete: {
            ...
            friends: [ 
                {
                    # deletes Bob and all relationships between Bob and all other nodes (including the KNOWS relationships)
